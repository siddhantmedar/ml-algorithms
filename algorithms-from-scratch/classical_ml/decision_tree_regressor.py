...
class DecisionTree:
    ...
    def _best_split(self, X, y):
        ...
        for feature in range(n_features):
            ...
            for threshold in thresholds:
                ...
                mse_score = (y_left.shape[0] / y.shape[0]) * self._compute_mse(
                    y_left
                ) + (y_right.shape[0] / y.shape[0]) * self._compute_mse(y_right)
    ...
    def _build_tree(self, X, y, depth=0):
        ...
        if (
            ...
            node.value_prediction = np.mean(y)
            return node
    ...
    def _traverse_tree(self, x, node):
        # base case
        if node.value_prediction is not None:
            return node.value_prediction
    ...
    def print_tree(self, node=None, depth=0):
        ...
        if node.value_prediction is not None:
            print(f"{indent}Predict: {node.value_prediction}")
            return
...
if __name__ == "__main__":
    ...
    print("MSE:", compute_mse(y_test, y_pred))
    print("RMSE:", compute_rmse(y_test, y_pred))
    print("MAE:", compute_mae(y_test, y_pred))
