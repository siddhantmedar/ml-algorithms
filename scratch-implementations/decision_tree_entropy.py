...
class DecisionTree:
    ...
    def _compute_entropy(self, y):
        ...
        entropy_score = -np.sum(probs * np.log2(probs + 1e-15))
    ...
    def _best_split(self, X, y):
        ...
        for feature in range(n_features):
            ...
            for threshold in thresholds:
                ...
                entropy_score = (y_left.shape[0] / y.shape[0]) * self._compute_entropy(
                    y_left
                ) + (y_right.shape[0] / y.shape[0]) * self._compute_entropy(y_right)
