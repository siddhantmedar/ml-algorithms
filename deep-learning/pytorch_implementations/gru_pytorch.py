...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for batch_index, (batch_data, batch_target) in enumerate(train_dataloader):
            ...
            print(
                f"Batch {batch_index}: Input batch shape: {batch_data.shape}, Target batch shape: {batch_target.shape}, Device: {batch_data.device}"
            )
    ...
    plt.ylabel("Loss")
    plt.savefig(
        f"./plots/training_loss_{epochs}_epochs.png"
    )  # Save the plot to a relative path
    plt.show()
